...
const accountLabel = (code: string) => `${code} · ${accountByCode[code]?.name || "Unknown account"}`;
...
export default function CompanyAccounting({ view, receipts, onToast }: Props) {
  const [entries, setEntries] = useState<JournalEntry[]>(seedJournalEntries);
  ...
  useEffect(() => {
    try {
      const saved = window.localStorage.getItem("ams-company-journals-v1");
      const lock = window.localStorage.getItem("ams-company-period-2026-locked");
      if (saved) {
      ...
      // Built-in MPERS example ledger remains available when storage is blocked.
    } finally {
      setReady(true);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    setEntries((current) => {
      ...
      const additions = receipts.filter((receipt) => !linked.has(receipt.id)).map(createReceiptJournal);
  ...
  useEffect(() => {
    if (!ready) return;
    window.localStorage.setItem("ams-company-journals-v1", JSON.stringify(entries));
  }, [entries, ready]);

  const reports = useMemo(() => ({
    tb: trialBalance(entries),
    pnl: profitAndLoss(entries),
    balance: balanceSheet(entries),
    cashFlow: cashFlowStatement(entries),
    tax: taxComputation(entries),
  }), [entries]);
  ...
  function postEntry(id: string) {
    ...
    if (!entry || !validateJournal(entry)) return onToast("Journal cannot be posted: debit and credit must balance.");
    setEntries((current) => current.map((item) => item.id === id ? { ...item, status: "posted" } : item));
    onToast("Journal posted to the Solver Academy general ledger.");
  }
  ...
  function toggleLock() {
    ...
    window.localStorage.setItem("ams-company-period-2026-locked", String(next));
  ...
  return <div className="content company-accounting">
    ...
    {view === "pl" && <ProfitAndLossView report={reports.pnl} onExport={() => {
      ...
      saveDownload("AMS-Solver-Academy-Profit-and-Loss-YA2026.csv", ["Account code,Account,Amount MYR", ...rows].join("\n"));
      onToast("Profit & Loss exported as CSV.");
    }} />}

    {view === "balance" && <BalanceSheetView report={reports.balance} />}
    {view === "cashflow" && <CashFlowView report={reports.cashFlow} />}
    {view === "tax" && <CompanyTaxView report={reports.tax} />}

    {journalOpen && <JournalModal onClose={() => setJournalOpen(false)} onSave={addJournal} />}
  </div>;
}
...
function StatementTable({ rows, totalLabel }: { rows: { code?: string; name: string; debit?: number; credit?: number; amount?: number }[]; totalLabel: string }) {
  const hasDebitCredit = rows.some((row) => row.debit !== undefined || row.credit !== undefined);
...
function JournalModal({ onClose, onSave }: { onClose: () => void; onSave: (entry: JournalEntry) => void }) {
  ...
  const valid = validateJournal({ lines }) && reference.trim() && description.trim();
