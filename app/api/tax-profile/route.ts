...
function userId(request: NextRequest) {
  return request.headers.get("x-openai-user-id") || "local-preview";
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    return NextResponse.json({ checklist: JSON.parse(row.checklist_json), formType: row.form_type, updatedAt: row.updated_at });
  } catch {
...
export async function POST(request: NextRequest) {
  const body = await request.json().catch(() => null) as { entity?: unknown; year?: unknown; formType?: unknown; checklist?: unknown } | null;
  ...
  await db.prepare(`INSERT INTO tax_profiles (id, user_id, entity_type, year, form_type, checklist_json, updated_at)
    ...
    .bind(id, owner, body.entity, year, expectedForm, JSON.stringify(body.checklist), updatedAt).run();
